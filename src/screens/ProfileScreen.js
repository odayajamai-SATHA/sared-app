...
export default function ProfileScreen({ navigation: rawNav }) {
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share(Platform.OS === 'ios' ? { message: t('shareMessage'), url: appLink } : { message });
    } catch {}
  };
  ...
  const menuGroups = [
    ...
    },
    {
      items: [
        ...
        { icon: 'help-circle-outline', label: t('helpSupport'), color: colors.darkGray },
        { icon: 'document-text-outline', label: t('termsConditions'), color: colors.darkGray },
        { icon: 'settings-outline', label: lang === 'ar' ? 'الإعدادات' : 'Settings', color: colors.darkGray, screen: 'Settings' },
      ],
    },
  ];
